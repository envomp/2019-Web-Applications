...
<?php if (is_logged_in() || (defined("INCLUDE_HEADER") && INCLUDE_HEADER)) { ?>
    <div id="header">
        <ul id="left-navbar" class="navbar header-item">
            <li class="navitem">
                <a<?php if (WEBPAGE_CONTEXT == "index.php") {
                    echo " class=\"current\"";
                } ?> href="<?php echo SITE_ROOT; ?>">Home</a>
            </li><!--
                <?php if (is_logged_in()) { ?>
	     -->
            <li class="navitem">
	 ...
	 -->
        <div id="search-wrapper" class="header-item">
            <form action="<?php echo SITE_ROOT . "/search.php"; ?>" id="search-form" name="search-form" method="get"
                  onsubmit="return document.forms['search-form']['q'].value.trim().length > 0;">
                <input type="text" id="search-input" placeholder="Insert * to get all users" name="q" value="<?php if (isset($_GET['q'])) {
                    echo htmlentities($_GET['q']);
                } ?>"/><input type="submit" class="submit-button" id="search-submit" value="Search"/>
            </form>
        </div><!--
	 ...
	 -->
        <ul id="right-navbar" class="navbar header-item">
            <li class="navitem">
                <a<?php if (WEBPAGE_CONTEXT == "edit-profile.php") {
                    echo " class=\"current\"";
                } ?> href="<?php echo SITE_ROOT . "/edit-profile.php"; ?>">Edit Profile</a>
            </li><!--
	     -->
            <li class="navitem">
