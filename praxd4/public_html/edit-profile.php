...
if (isset($_POST['edit-photo-submitted'])) {
    ...
    if (!$ERRORS) {
        if (move_uploaded_file($_FILES['profile-photo']['tmp_name'], __DIR__ . "/images/profile/" . $_SESSION['user']['id'] . ".png")) {
            $db->query("UPDATE `users_185787_envomp` SET `photo`=1 WHERE `user_id`=" . $db->real_escape_string((int)$_SESSION['user']['id']));
    ...
    if ($name != $_SESSION['user']['name']) {
        if (!$name) {
            ...
            $query = "UPDATE `users_185787_envomp` SET `name`='" . $db->real_escape_string($name) . "', `date_updated`=NOW() WHERE `user_id`=" . $db->real_escape_string($user_id);
    ...
    if ($handle != $_SESSION['user']['handle']) {
        if (!$handle) {
            ...
            $query = "UPDATE `users_185787_envomp` SET `handle`='" . $db->real_escape_string($handle) . "', `date_updated`=NOW() WHERE `user_id`=" . $db->real_escape_string($user_id);
            $result = $db->query($query);
            if ($result) {
    ...
    if ($bio != $_SESSION['user']['bio']) {
        if (strlen($bio) > 512) {
            ...
            $query = "UPDATE `users_185787_envomp` SET `bio`='" . $db->real_escape_string($bio) . "', `date_updated`=NOW() WHERE `user_id`=" . $db->real_escape_string($user_id);
            $result = $db->query($query);
            if ($result) {
    ...
    if ($email != $_SESSION['user']['email']) {
        if (!$email) {
            ...
            $query = "UPDATE `users_185787_envomp` SET `email`='" . $db->real_escape_string($email) . "', `date_updated`=NOW() WHERE `user_id`=" . $db->real_escape_string($user_id);
            $result = $db->query($query);
            if ($result) {
    ...
    if (!$current_password) {
        ...
        $query = "UPDATE `users_185787_envomp` SET `password`='" . $db->real_escape_string($new_password_hash) . "', `date_updated`=NOW() WHERE `user_id`=" . $db->real_escape_string($user_id);
        $result = $db->query($query);
        if ($result) {
    ...
    $query = "DELETE FROM `users_185787_envomp` WHERE `user_id`=" . $db->real_escape_string($_SESSION['user']['id']);
    $result = $db->query($query);
    if ($result) {
        logout();
...
?>
    ...
    <div class="form-wrapper box">
        ...
        if ($_SESSION['user']['photo']) {
            ...
            echo "<img src=\"" . SITE_ROOT . "/images/profile/" . $_SESSION['user']['id'] . ".png\" width=\"200px\" height=\"auto\" />\n";
            echo "</div>\n";
        }
        ?>
        ...
        <form id="edit-form" name="edit-form" method="post">
            ...
            <div class="input-wrapper">
                ...
                <input class="text-input" type="text" id="edit-handle" name="edit-handle"
                       onkeydown="updateHandleAvailability('edit-handle', 'edit-handle-available');"
                       onpaste="javascript:updateHandleAvailability('edit-handle', 'edit-handle-available');"
                       oninput="updateHandleAvailability('edit-handle', 'edit-handle-available');"
                       onblur="updateHandleAvailability('edit-handle', 'edit-handle-available');"
