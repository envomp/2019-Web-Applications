...
if (isset($_POST['register-submitted'])) {
    ...
    if (!$register_handle) {
        ...
        $ERRORS[] = "Username cannot be more than 20 characters.";
    } else if (get_user_by_handle($register_handle)) {
        $ERRORS[] = "The username \"$register_handle\" is already registered.";
    }
    ...
    if (!count($ERRORS)) {
        ...
        $query = "INSERT INTO `users_185787_envomp`
        ...
        if ($result) {
            if ($_FILES['register-photo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['register-photo']['tmp_name'], __DIR__ . "/images/profile/" . $db->insert_id . ".png")) {
                    $db->query("UPDATE `users_185787_envomp` SET `photo`=1 WHERE `user_id`=" . $db->real_escape_string((int)$user_id));
                } else {
...
?>
    ...
    <div class="form-wrapper box">
        ...
        <form id="register-form" name="register-form" method="post" enctype="multipart/form-data">
            ...
            <div class="input-wrapper">
                <label for="register-handle" class="input-required">Username</label>
                <input class="text-input" type="text" id="register-handle" name="register-handle"
                       onkeydown="updateHandleAvailability('register-handle', 'register-handle-available');"
                       onpaste="javascript:updateHandleAvailability('register-handle', 'register-handle-available');"
                       oninput="updateHandleAvailability('register-handle', 'register-handle-available');"
                       onblur="updateHandleAvailability('register-handle', 'register-handle-available');"
            ...
            <div class="submit-wrapper">
                ...
                or
                <a class="register-ref" href="<?php echo SITE_ROOT . "/login.php"; ?>">Cancel</a>
            </div>
        </form>
    </div>
